      ...
      <p-card>
        <ng-template pTemplate="header">
          <div class="flex items-center gap-3 p-4">
            ...
            <div>
              <h2 class="text-xl font-bold text-gray-900 m-0">mark</h2>
              <p class="text-gray-600 text-sm m-0">Ullamco est placeat</p>
            </div>
          </div>
        </ng-template>
        ...
        <div class="tool-content">
          @if (loading()) {
            <p-message severity="info" text="Loading mark..." [closable]="false"></p-message>
          } @else if (error()) {
            <p-message
              severity="error"
            ...
            <div class="text-center py-8">
              ...
              <p class="text-gray-600 mb-4">The mark tool is ready for implementation.</p>
              <p class="text-sm text-gray-500">Tool Key: mark | Component: MarkComponent</p>
...
export class MarkComponent implements OnInit {
  ...
  private async initializeTool(): Promise<void> {
    try {
      ...
      await this.markService.initialize();
    } catch (error) {
      console.error('Failed to initialize mark:', error);
