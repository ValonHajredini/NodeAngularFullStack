...
@Component({
  ...
  imports: [CommonModule, DecimalPipe, BarChartComponent, PieChartComponent, Message, Divider],
  template: `
    <div
    ...
    >
      ...
      @if (hasSubmissions()) {
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <!-- Pass/Fail Pie Chart -->
          <div class="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
            <app-pie-chart [title]="'Pass vs Fail Rate'" [data]="passFailData()"></app-pie-chart>
          </div>
        </div>
        ...
        <div
          ...
          role="status"
          aria-label="No quiz submissions yet"
        >
          <p-message
            severity="info"
      ...
      <div class="sr-only" aria-live="polite" aria-atomic="true">
        ...
        {{ metrics().averageScore | number: '1.1-1' }}%, median {{ metrics().medianScore }}%, pass
        rate {{ metrics().passRate | number: '1.1-1' }}%. Score range from
...
export class QuizAnalyticsComponent {
   ...
   */
  hasQuestionAccuracy = computed(() => Object.keys(this.metrics().questionAccuracy).length > 0);
