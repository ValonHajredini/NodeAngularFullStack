...
/**
 * Polar chart component for displaying choice field distributions in circular segments.
    ...
    <div class="chart-container" role="region" [attr.aria-label]="'Chart: ' + title()">
      ...
      @if (data().length > 0) {
        @if (!showDataTable()) {
          ...
          <div class="sr-only" role="table" aria-label="Data table alternative for polar chart">
            <table>
              <caption>
                {{
                  title()
...
export class PolarChartComponent {
  ...
  /** Chart colors for polar segments */
  ...
  getChartAriaLabel(): string {
    ...
    return `Polar area chart for ${this.title()}. Total responses: ${totalResponses}. Most popular option: ${topOption.label} with ${topOption.count} responses (${topOption.percentage.toFixed(1)}%).`;
  }
}
