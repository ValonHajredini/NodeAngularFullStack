...
export class FormsUploadController {
  /**
   * Multer middleware for single file upload (background images)
   */
  uploadMiddleware = upload.single('backgroundImage');

  /**
   * Multer middleware for form field image upload
   */
  uploadFieldImageMiddleware = upload.single('image');
  ...
  /**
   * Uploads a form field image to DigitalOcean Spaces.
   * @route POST /api/forms/:formId/upload-image
   * @param req - Express request object with file
   * @param res - Express response object
   ...
   * @throws {ApiError} 500 - Upload failed
   * @example
   * POST /api/forms/:formId/upload-image
   * Content-Type: multipart/form-data
   * Authorization: Bearer <token>
   * Body: { image: <file> }
   */
  uploadFormImage = AsyncHandler(
    async (req: AuthRequest, res: Response): Promise<void> => {
      const userId = req.user?.id;
      ...
      if (!formId) {
        throw new ApiError('Form ID is required', 400, 'FORM_ID_REQUIRED');
      }
      ...
      // Validate file type (additional validation beyond multer)
      const allowedMimeTypes = [
        'image/jpeg',
        'image/png',
        'image/gif',
      ...
      // Generate unique filename with form ID and timestamp
      const timestamp = Date.now();
      const extension = file.originalname.split('.').pop() || 'jpg';
      const fileName = `form-images/${userId}/${formId}/${timestamp}.${extension}`;
