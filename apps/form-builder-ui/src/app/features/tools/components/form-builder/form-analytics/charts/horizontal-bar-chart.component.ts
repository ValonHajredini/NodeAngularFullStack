...
/**
 * Horizontal bar chart component for displaying choice distribution data.
 * Best for choice fields with long option labels.
 * Labels on Y-axis (left), bars extend rightward for better label readability.
      ...
      @if (data().length > 0) {
        @if (!showDataTable()) {
          ...
          <div
            class="sr-only"
            role="table"
            aria-label="Data table alternative for horizontal bar chart"
          >
            <table>
              <caption>
                {{
...
export class HorizontalBarChartComponent {
  ...
  chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (context: { dataIndex: number; parsed: { x: number } }): string => {
            const distribution = this.data()[context.dataIndex];
            const DECIMAL_PLACES = 1;
            return `${context.parsed.x} responses (${distribution.percentage.toFixed(DECIMAL_PLACES)}%)`;
          },
        },
      },
    },
  ...
  getChartAriaLabel(): string {
    ...
    return `Horizontal bar chart for ${this.title()}. Total responses: ${totalResponses}. Most popular option: ${topOption.label} with ${topOption.count} responses (${topOption.percentage.toFixed(1)}%).`;
  }
}
