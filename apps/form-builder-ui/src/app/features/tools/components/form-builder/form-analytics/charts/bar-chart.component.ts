...
/**
 * Bar chart component for displaying choice field distributions.
 * Shows count and percentage for each option with interactive tooltips.
    ...
    <div class="chart-container" role="region" [attr.aria-label]="'Chart: ' + title()">
      ...
      @if (data().length > 0) {
        @if (!showDataTable()) {
          ...
          <div class="sr-only" role="table" aria-label="Data table alternative for bar chart">
            <table>
              <caption>
                {{
                  title()
...
export class BarChartComponent {
      ...
      tooltip: {
        callbacks: {
          label: (context: { dataIndex: number; parsed: { y: number } }): string => {
            ...
            return `${context.parsed.y} responses (${distribution.percentage.toFixed(DECIMAL_PLACES)}%)`;
          },
        },
      },
    },
  ...
  getChartAriaLabel(): string {
    ...
    return `Bar chart for ${this.title()}. Total responses: ${totalResponses}. Most popular option: ${topOption.label} with ${topOption.count} responses (${topOption.percentage.toFixed(1)}%).`;
  }
}
