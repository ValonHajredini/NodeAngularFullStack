...
/**
 * Doughnut chart component for displaying distribution data as hollow circular segments.
    ...
    <div class="chart-container" role="region" [attr.aria-label]="'Chart: ' + title()">
      ...
      @if (data().length > 0) {
        @if (!showDataTable()) {
          ...
          <div class="sr-only" role="table" aria-label="Data table alternative for doughnut chart">
            <table>
              <caption>
                {{
                  title()
...
export class DoughnutChartComponent {
  ...
  /** Chart colors for doughnut segments */
    ...
    datasets: [
      {
        ...
        backgroundColor: this.CHART_COLORS.slice(0, this.data().length),
        borderColor: '#FFFFFF',
        borderWidth: 2,
      },
    ],
      ...
      tooltip: {
        callbacks: {
          label: (context: { dataIndex: number; parsed: number }): string => {
            ...
            return `${distribution.label}: ${context.parsed} responses (${distribution.percentage.toFixed(DECIMAL_PLACES)}%)`;
          },
        },
      },
    },
  ...
  getChartAriaLabel(): string {
    ...
    return `Doughnut chart for ${this.title()}. Total responses: ${totalResponses}. Most popular option: ${topOption.label} with ${topOption.count} responses (${topOption.percentage.toFixed(1)}%).`;
  }
}
