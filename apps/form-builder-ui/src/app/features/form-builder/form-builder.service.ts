...
export class FormBuilderService {
  ...
  applyTheme(theme: FormTheme): void {
    ...
    this.themesApi.applyTheme(theme.id).subscribe({
      next: (response: any) => console.log('Theme usage tracked:', response.data?.usageCount),
      error: (err: any) => console.error('Failed to track theme usage:', err),
    });
  }
