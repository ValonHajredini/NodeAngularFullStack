 ...
 */
export const generateValidUserData = (overrides: Partial<TestUser> = {}): TestUser => {
...
export const generateEdgeCaseData = () => {
  return {
    ...
    boundaryValues: {
      ...
      passwords: {
        shortest: 'Aa1!',
        longest: 'A'.repeat(64) + 'a'.repeat(32) + '1'.repeat(16) + '!'.repeat(16)
      },
      names: {
        shortest: 'A',
        longest: 'A'.repeat(50)
    ...
    largeDataSets: {
      generateUsers: (count: number) => {
        return Array.from({ length: count }, (_, i) => generateValidUserData({
