...
- [ ] Task 1: Update FormsRepository to handle theme references (AC: 1, 2, 3)
  - [ ] Open file `apps/api/src/repositories/forms.repository.ts`
  - [ ] Modify `create(formData)` method to accept `theme_id` column in INSERT statement
  - [ ] Modify `update(id, updates)` method to accept `theme_id` in UPDATE statement
  - [ ] Modify `findById(id)` method to LEFT JOIN form_themes table and return embedded theme:
    ```sql
    SELECT
      form_schemas.*,
    ...
    ```
  - [ ] Transform nested theme columns into theme object in result mapping
  - [ ] Verify parameterized queries prevent SQL injection

- [ ] Task 2: Update FormsService to validate and manage theme references (AC: 5)
  - [ ] Open file `apps/api/src/services/forms.service.ts`
  - [ ] Inject `ThemesRepository` dependency in constructor
  - [ ] Modify `createForm(formData, userId)` method:
  ...
  - [ ] Modify `updateForm(id, updates, userId)` method:
  ...
  - [ ] Modify `getFormById(id, userId)` method:
    - Call repository.findById() which now returns embedded theme
    - Return form object with theme property
  - [ ] Add JSDoc comments documenting theme validation logic

- [ ] Task 3: Update PublicFormsService to include themes (AC: 4)
  - [ ] Open file `apps/api/src/services/public-forms.service.ts` (or equivalent)
  - [ ] Modify `getPublicForm(shortCode)` method to fetch form with theme:
    - Use same LEFT JOIN pattern as forms repository
    - Return form schema with embedded theme object
  - [ ] Ensure public forms with deleted themes (theme_id references inactive theme) still load with
        theme = null

- [ ] Task 4: Update FormsValidator to validate themeId (AC: 5)
  - [ ] Open file `apps/api/src/validators/forms.validator.ts`
  - [ ] Add validation rule to `validateCreateForm`:
    ```typescript
    body('themeId').optional().isUUID().withMessage('Theme ID must be a valid UUID');
    ```
  - [ ] Add same validation to `validateUpdateForm`
  - [ ] Service layer will handle business rule validation (theme exists and active)

- [ ] Task 5: Update FormsController to handle theme responses (AC: 3, 4)
  - [ ] Open file `apps/api/src/controllers/forms.controller.ts`
  - [ ] Modify `createForm` handler:
    - Accept `themeId` from req.body
    - Pass to service.createForm()
    - Return created form with theme in response
  - [ ] Modify `updateForm` handler:
    - Accept `themeId` from req.body (null to remove theme)
    - Pass to service.updateForm()
    - Return updated form with theme
  - [ ] Modify `getForm` handler:
    - Response now includes theme property if themeId is set
    - Return 200 with { success: true, data: { ...form, theme: {...} } }
  - [ ] Verify JSON serialization handles Date fields (createdAt, updatedAt) correctly

- [ ] Task 6: Update PublicFormsController (AC: 4)
  - [ ] Open file `apps/api/src/controllers/public-forms.controller.ts`
  - [ ] Modify `getPublicForm` handler:
...
- [ ] Task 7: Update Swagger documentation (AC: All)
  - [ ] Update JSDoc for POST /api/forms:
    ```typescript
    /**
     * @swagger
     * /api/forms:
     ...
     */
    ```
  - [ ] Update JSDoc for PATCH /api/forms/:id to include themeId in requestBody
  - [ ] Update JSDoc for GET /api/forms/:id response to show theme object
  - [ ] Update JSDoc for GET /api/public/forms/:shortCode response to show theme
  - [ ] Test Swagger UI shows updated request/response schemas

- [ ] Task 8: Write integration tests (AC: 6, IV1, IV2, IV3)
  - [ ] Open file `apps/api/tests/integration/forms.test.ts`
  - [ ] Add test: "should create form with valid theme ID"
