...
- [x] Service follows Clean Architecture principles (service → repository pattern)
...
- [ ] Unit tests for ExportOrchestratorService methods (≥90% coverage) - DEFERRED (Task 16)
- [x] Unit tests for each export strategy (≥85% coverage)
...
3. [x] Map tool types to strategy classes (forms → FormsExportStrategy, etc.)
...
- ✅ Export routes accessible at `/api/tool-registry/export-jobs/:jobId`
- ✅ Authentication middleware working (returns 401 Unauthorized for unauthenticated requests)
- ✅ Rate limiting configured correctly with IPv6 support
...
- Story 33.1.2 (Export Job Database Schema) ✅ Already completed
- Story 33.1.3 (Export Job Status Tracking) ✅ Can now proceed (dependency satisfied)
