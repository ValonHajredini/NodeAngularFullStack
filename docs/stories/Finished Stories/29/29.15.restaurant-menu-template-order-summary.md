...
**Epic**: Epic 29 - Form Template System with Business Logic **Story ID**: 29.15 **Status**:
Approved **Story Points**: 6 **Priority**: Medium **Risk Level**: Low (In-memory calculation,
similar to quiz template) **Dependencies**: Stories 29.1-29.5 (Backend Foundation), 29.11 (Base
Executor Interface) **Related Stories**: 29.11-29.14 (Other Template Types)
...
**🚨 CRITICAL**: Story 29.15 has **NOT been implemented**. This is a specification-only review.

**Evidence**:

- No `OrderExecutor` class exists (expected:
  `apps/forms-api/src/services/template-executors/order-executor.ts`)
- No `OrderSummaryComponent` exists (expected:
  `apps/form-builder-ui/src/app/features/public/form-renderer/order-summary/`)
- No restaurant menu seed file exists (expected:
  `apps/forms-api/database/seeds/035_seed_restaurant_menu_template.ts`)
- Template executor registry shows commented placeholder for order executor (line 55)
- Story status: "Approved" but implementation phase has not started

**Implemented Templates**: inventory, appointment, quiz, poll ✓

### Specification Quality Assessment

**Overall Rating**: 7.5/10 - High-quality specification with one **critical blocker** and minor gaps

**Strengths**:

1. ✓ **Excellent Architecture**: Follows QuizExecutor pattern correctly (synchronous, in-memory
   calculation)
2. ✓ **Comprehensive Implementation Guide**: Detailed code examples for all components
3. ✓ **Currency Handling**: Cents-based arithmetic avoids floating-point precision issues
4. ✓ **Performance Requirements**: Clear targets (< 50ms backend, < 16ms frontend)
5. ✓ **Test Specifications**: Detailed unit test scenarios with expected values
6. ✓ **Pattern Consistency**: Matches ITemplateExecutor interface and existing executor patterns
7. ✓ **UX Considerations**: Two-column layout, sticky summary, mobile responsiveness addressed

### Critical Issues Found

#### 1. 🚨 **CRITICAL - Type Specification Mismatch** (BLOCKS IMPLEMENTATION)

**Severity**: HIGH **Priority**: MUST FIX BEFORE IMPLEMENTATION

**Problem**: Story specifies detailed types (`OrderLogicConfig`, `MenuItemConfig`, `OrderMetadata`)
but shared types package contains only simplified `OrderConfig`.

**Current State** (`packages/shared/src/types/templates.types.ts:247-258`):

```typescript
export interface OrderConfig {
  type: 'order';
  itemFields: string[]; // ❌ Simple string array
  calculateTotal: boolean;
  taxRate?: number;
  shippingField?: string; // ❌ Missing serviceFee, currency, itemFields details
}
```

**Story Specification** (lines 82-105, 108-124):

```typescript
export interface MenuItemConfig {
  fieldId: string;
  quantityFieldId: string;
  price: number;               // ✓ Per-item pricing
  name: string;
  category?: string;
}
...
export interface OrderLogicConfig {
  type: 'order';
  itemFields: MenuItemConfig[]; // ✓ Detailed item configs
  calculateTotal: boolean;
  currency?: string;             // ✓ Currency support
  taxRate?: number;
  serviceFee?: number;           // ✓ Service fee support
}
...
export interface OrderMetadata {
  ...
  service_fee?: number;
  total: number;
  items: Array<{...}>;         // ✓ Detailed item breakdown
  ordered_at: string;
}
```
...
- TypeScript compilation will fail when implementing OrderExecutor
- Type narrowing in `TemplateBusinessLogicConfig` union won't work correctly
- Frontend OrderSummaryComponent can't access per-item pricing metadata

**Resolution Required**:

1. **Option A (RECOMMENDED)**: Update `packages/shared/src/types/templates.types.ts` to match story
   specification
   - Replace `OrderConfig` with `OrderLogicConfig`
   - Add `MenuItemConfig` interface
   - Add `OrderMetadata` interface
   - Update `TemplateBusinessLogicConfig` union type
   - Run `npm run build:shared` to rebuild package

2. **Option B**: Simplify story specification to match existing types (NOT recommended - loses
   critical functionality)

**Recommendation**: Implement Option A before starting development. The detailed types are necessary
for per-item pricing and order totals calculation.

#### 2. **MEDIUM - Missing Integration Test Specification**

**Severity**: MEDIUM **Task Reference**: Task 10 (line 1347)

**Problem**: Task 10 states "Write backend integration tests for order submission flow" but provides
no specification for what the integration test should validate.

**Missing Details**:

- End-to-end flow: public form submission → executor validation → order total calculation → metadata
  storage
- Database transaction rollback behavior on execution failure
- Concurrent order submission handling
- Integration with FormsService submission workflow

**Recommendation**: Add integration test specification in Dev Notes section detailing:

- Test scenario: User submits order via public form endpoint
- Expected: Order total calculated and stored in `form_submissions.metadata` JSONB
- Assertions: Validate metadata structure matches `OrderMetadata` interface

#### 3. **LOW - Analytics Method Test Coverage Gap**

**Severity**: LOW **Task References**: Task 7, Task 8 (lines 1344-1345)

**Problem**:

- Task 7: "Extend StatisticsEngineService with `analyzeOrderRevenue()` method"
- Task 8: "Update FormAnalyticsComponent to display order revenue statistics"
- **Gap**: No unit tests specified for `analyzeOrderRevenue()` method itself, only UI display tests

**Code Provided** (lines 717-765): Method implementation but no test spec

**Recommendation**: Add test specification:

```typescript
describe('StatisticsEngineService.analyzeOrderRevenue', () => {
  it('should calculate total revenue across all orders', ...);
  it('should calculate average order value', ...);
  it('should identify top 10 items by revenue', ...);
  it('should handle empty submissions array', ...);
  it('should handle submissions without order metadata', ...);
});
```

#### 4. **LOW - Hard-coded Quantity Limit**

**Severity**: LOW **Location**: OrderExecutor validation (line 202)

**Problem**: 100-item quantity limit is hard-coded in executor validation:
...
**Issue**: Different restaurant types may need different limits (fast food vs catering)

**Recommendation**: Make configurable via `OrderLogicConfig`:

```typescript
export interface OrderLogicConfig {
  // ... existing fields
  maxQuantityPerItem?: number; // Default: 100
}
```

#### 5. **LOW - Missing Responsive Layout Test**

**Severity**: LOW **Task Reference**: Task 11 (line 1348)

**Problem**: Story specifies two-column layout that stacks on mobile (lines 688-708) but no test
...
**Recommendation**: Add component test specification:

```typescript
describe('FormRendererComponent - Order Layout', () => {
  it('should display two-column layout on desktop (>= 768px)', ...);
  it('should stack vertically on mobile (< 768px)', ...);
  it('should show sticky order summary on desktop', ...);
});
```
...
- ✓ Strategy Pattern correctly applied (ITemplateExecutor interface)
- ✓ JSONB metadata storage (no new tables)
- ✓ Synchronous execution pattern matches QuizExecutor
- ✓ Service layer integration via TemplateExecutorRegistry

**Coding Standards** (`docs/architecture/coding-standards.md`): ✓ PASS

- ✓ JSDoc comments present on all public APIs
- ✓ Type definitions in shared package (once mismatch fixed)
- ✓ Parameterized queries in seed script (SQL injection prevention)
- ✓ Input validation on backend and frontend

**Testing Strategy**: ⚠ CONCERNS

- ✓ Unit test scenarios well-defined
- ✓ Performance test specified (50ms target)
- ⚠ Integration test specification incomplete (Task 10)
- ⚠ Analytics method test coverage missing (Task 7)
- ⚠ Responsive layout test not specified (Task 11)
...
- ✓ No XSS risk (prices are numbers, not user input)
- ✓ Input validation prevents negative prices and excessive quantities
- ✓ No sensitive data in order metadata

**Performance**: ✓ PASS

- ✓ Clear target: < 50ms for backend calculation (P95)
- ✓ Frontend: < 16ms for UI updates (60fps)
- ✓ Synchronous calculation avoids async overhead
- ✓ Integer arithmetic (cents) is fast and precise

**Reliability**: ✓ PASS

- ✓ Error handling in validation method
- ✓ Transaction handling in seed script
- ✓ Graceful degradation (empty order state shown)

**Maintainability**: ✓ PASS (after type fix)

- ✓ Clear separation of concerns (executor, component, analytics)
- ✓ Comprehensive JSDoc comments
- ✓ Code examples facilitate understanding
- ⚠ Type mismatch will cause maintenance issues if not fixed

### Testability Evaluation

**Controllability**: ✓ HIGH

- All inputs controllable via form data
- Configuration injectable via OrderLogicConfig
- No external dependencies (API calls, timers)

**Observability**: ✓ HIGH

- Metadata stored in database (queryable)
- UI displays all calculated values
- Error messages specific and actionable

**Debuggability**: ✓ HIGH

- Synchronous execution (no async race conditions)
- Detailed metadata breakdown (per-item totals)
...
| AC  | Requirement                                   | Validation Method | Coverage | Gap                          |
| --- | --------------------------------------------- | ----------------- | -------- | ---------------------------- |
| AC1 | Template created with category SERVICES       | Seed Data         | ✓        | -                            |
| AC2 | Schema: CHECKBOX + NUMBER fields with pricing | Seed Data         | ✓        | -                            |
| AC3 | Items include metadata.price                  | Unit Tests        | ✓        | -                            |
| AC4 | Business logic config with order type         | Unit Tests        | ✓        | Type mismatch issue          |
| AC5 | OrderExecutor strategy class created          | Unit Tests        | ✓        | Not implemented yet          |
| AC6 | Calculates total with formula                 | Unit Tests        | ✓        | Detailed test specs provided |
| AC7 | Stores total in submission metadata           | Integration Tests | ⚠       | Test spec incomplete         |
| AC8 | Frontend displays running total               | Component Tests   | ✓        | Detailed test specs provided |

**Integration Verification Coverage**:

| IV  | Verification                      | Test Method       | Coverage | Gap                               |
| --- | --------------------------------- | ----------------- | -------- | --------------------------------- |
| IV1 | Existing fields unchanged         | Regression Tests  | ✓        | Relies on existing test suite     |
| IV2 | Calculation < 50ms                | Performance Tests | ✓        | Test spec provided (line 965-986) |
| IV3 | Order total displays in analytics | Component Tests   | ⚠       | Analytics method test missing     |

**Coverage Score**: 8/11 fully specified (73%) **Gap Score**: 3 items need additional specification

### Technical Debt Assessment

**Immediate Debt** (if implemented as-is):

1. Type mismatch between story spec and shared types → compilation errors
2. Missing integration test specification → incomplete test coverage
3. Hard-coded quantity limit → inflexibility for different use cases

**Future Considerations**:

1. Currency formatting logic duplicated (backend + frontend) → consider shared utility
2. No localization support for currency formatting → i18n not addressed
3. Tax calculation uses `Math.round()` → rounding strategy not documented (always round down? round
   up? banker's rounding?)

**Estimated Remediation Effort**:

- Type mismatch fix: 1 hour (update shared types + rebuild)
- Integration test spec: 0.5 hours (document test scenario)
- Configurable quantity limit: 0.5 hours (add field to config interface)
- **Total**: 2 hours to address all concerns

### Recommendations

#### Immediate (Before Implementation Starts):

1. **FIX TYPE MISMATCH** (Priority: CRITICAL)
   - Action: Update `packages/shared/src/types/templates.types.ts`
   - Add: `MenuItemConfig`, `OrderLogicConfig`, `OrderMetadata` interfaces
   - Replace: `OrderConfig` → `OrderLogicConfig` in union type
   - Verify: Run `npm run build:shared && npm run typecheck`
   - Refs: [`packages/shared/src/types/templates.types.ts:247-282`]

2. **SPECIFY INTEGRATION TEST** (Priority: HIGH)
   - Action: Add integration test specification to Dev Notes
   - Include: End-to-end order submission flow validation
   - Refs: [Task 10, line 1347]

3. **ADD ANALYTICS METHOD TESTS** (Priority: MEDIUM)
   - Action: Add unit test specification for `analyzeOrderRevenue()`
   - Include: Revenue calculation, average order value, top items by revenue
   - Refs: [Task 7, lines 717-765]

#### Future Enhancements:

4. **MAKE QUANTITY LIMIT CONFIGURABLE** (Priority: LOW)
   - Action: Add `maxQuantityPerItem` field to `OrderLogicConfig`
   - Default: 100 items
   - Refs: [OrderExecutor validation, line 202]

5. **SPECIFY RESPONSIVE LAYOUT TESTS** (Priority: LOW)
   - Action: Add component test specification for mobile layout
   - Include: Desktop two-column, mobile stacking
   - Refs: [Task 11, CSS lines 688-708]

6. **DOCUMENT ROUNDING STRATEGY** (Priority: INFO)
   - Action: Document tax rounding approach (currently `Math.round()`)
   - Consider: Regulatory requirements for tax calculation
   - Refs: [OrderExecutor line 270]

### Gate Status

Gate: **CONCERNS** → `docs/qa/gates/29.15-restaurant-menu-template-order-summary.yml`

**Reason**: Critical type specification mismatch blocks implementation. Specification is otherwise
high-quality and comprehensive. Must fix shared types before development starts.

### Recommended Status

**❌ Changes Required** - Address critical type mismatch before implementation

**Next Steps**:

1. Developer updates `packages/shared/src/types/templates.types.ts` to match story specification
2. Developer adds integration test specification to Dev Notes
3. Developer adds analytics method test specification
4. Quinn re-reviews updated specification → Gate: PASS
5. Story moves to "Ready for Implementation"

**Note**: Story owner decides final status transition. This is an advisory review only.
...
| ----------------- | ------- | ---------------- | ---------------------------------------- |
| Unit Tests        | Pending | Target: 90%+     | Comprehensive specs provided             |
| Integration Tests | Pending | -                | Spec incomplete (Task 10)                |
| Component Tests   | Pending | Target: 85%+     | Specs provided, responsive tests missing |
| Performance Tests | Pending | Target: 50ms P95 | Test spec provided (line 965-986)        |
