...
- [ ] Task 1: Remove Epic 22 Admin Theme Designer Files (AC: 1, 4)
  - [ ] Delete `apps/web/src/app/features/admin/pages/theme-designer/` directory
  - [ ] Delete `apps/web/src/app/features/admin/pages/theme-management/` directory
  - [ ] Delete `apps/web/src/app/features/admin/components/theme-preview/` directory
  - [ ] Delete `apps/web/src/app/features/admin/components/theme-export-dialog/` directory
  - [ ] Delete `apps/web/src/app/features/admin/components/theme-import-dialog/` directory
  - [ ] Delete `apps/web/src/app/features/admin/services/theme-designer.service.ts`
  - [ ] Delete `apps/api/src/routes/admin-themes.routes.ts`
  - [ ] Delete `apps/api/src/controllers/admin-themes.controller.ts`
  - [ ] Remove admin theme routes from `apps/web/src/app/features/admin/admin.routes.ts`
  - [ ] Remove admin theme navigation from
        `apps/web/src/app/layouts/main-layout/navigation.service.ts`

- [ ] Task 2: Revert Epic 21 Template Changes (AC: 2)
  - [ ] Checkout
  ...
  - [ ] Checkout `apps/web/src/app/features/public/form-renderer/form-renderer.component.html` from
        main branch baseline (before Epic 21 changes)
  - [ ] Verify reverted templates compile without errors

- [ ] Task 3: Clean `theme-variables.css` (AC: 3)
  - [ ] Review `apps/web/src/styles/theme-variables.css` for incomplete utility classes
  - [ ] Remove partial utility classes (keep CSS variable definitions from Epic 20)
  - [ ] Keep Epic 20 theme variable structure intact (`:root` CSS variables)
  - [ ] Document what was removed in code comments

- [ ] Task 4: Verify Epic 20 Functionality (AC: 5)
  - [ ] Start development environment (`npm start`)
  - [ ] Navigate to Form Builder and verify theme dropdown displays 9 seeded themes
  - [ ] Select a theme from dropdown and verify form preview shows theme applied (even if
        incomplete)
  - [ ] Test API endpoint `GET /api/themes` returns all themes
  - [ ] Verify existing forms with themes continue rendering (degraded but functional)
