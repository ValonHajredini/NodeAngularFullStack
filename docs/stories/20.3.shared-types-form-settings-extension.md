...
- [ ] Task 1: Define FormTheme interface in shared types (AC: 1)
  - [ ] Open file `packages/shared/src/types/forms.types.ts`
  - [ ] Add `FormTheme` interface with all properties:
    ```typescript
    ...
    ```
  - [ ] Add JSDoc comments describing each property [Source:
        coding-standards.md#documentation-standards]
  - [ ] Follow existing naming conventions: camelCase for properties [Source:
        coding-standards.md#naming-conventions]

- [ ] Task 2: Define ResponsiveThemeConfig and ThemeProperties interfaces (AC: 2)
  - [ ] Add `ResponsiveThemeConfig` interface in same file:
    ```typescript
    ...
    ```
  - [ ] Add `ThemeProperties` interface with all theme property fields:
    ```typescript
    ...
    ```
  - [ ] Add JSDoc comments for each interface and property
  - [ ] Document color properties expect hex format (#RRGGBB)
  - [ ] Document containerOpacity range (0.0 to 1.0)

- [ ] Task 3: Extend FormSettings interface with themeId (AC: 3, IV1)
  - [ ] Locate existing `FormSettings` interface in `packages/shared/src/types/forms.types.ts`
  - [ ] Add optional `themeId` property:
    ```typescript
    ...
    ```
  - [ ] Add JSDoc comment: `/** Optional theme ID reference for styling */`
  - [ ] Ensure backward compatibility: themeId is optional, existing code doesn't break

- [ ] Task 4: Update form response types to include theme (AC: 5)
  - [ ] Locate `FormSchema` interface in shared types
  - [ ] Update JSDoc to note that API responses may include embedded theme:
    ```typescript
    ...
    ```
  - [ ] Add JSDoc comment: `/** Embedded theme object when fetched from API (not stored in DB) */`
  - [ ] This allows API to return theme alongside form schema

- [ ] Task 5: Build shared package and verify no errors (AC: 4)
  - [ ] Run `npm run build:shared` from project root
  - [ ] Verify build completes without TypeScript errors
  - [ ] Verify output in `packages/shared/dist/` contains .js and .d.ts files
  - [ ] Check generated .d.ts files include FormTheme, ResponsiveThemeConfig, ThemeProperties
        exports
  - [ ] Run `npm --workspace=packages/shared run typecheck` to verify strict mode compliance

- [ ] Task 6: Update backend imports and verify compilation (AC: 5, IV3)
  - [ ] Update `apps/api/src/repositories/themes.repository.ts` to import from shared:
    ```typescript
    import { FormTheme } from '@nodeangularfullstack/shared';
    ```
  - [ ] Update `apps/api/src/services/themes.service.ts` to use shared types
  - [ ] Update `apps/api/src/controllers/themes.controller.ts` to reference shared types
  - [ ] Run `npm --workspace=apps/api run typecheck` to verify no errors
  - [ ] Verify API response serialization handles Date fields correctly (convert to ISO strings)

- [ ] Task 7: Update frontend imports and verify compilation (IV2)
  - [ ] (Placeholder for Story 20.5) Verify form-builder.service.ts can import FormTheme from shared
  - [ ] Run `npm --workspace=apps/web run typecheck` to verify no errors
  - [ ] Check that existing FormSettings usage compiles (backward compatibility check)

- [ ] Task 8: Write TypeScript type tests (AC: 6)
  - [ ] Create file `packages/shared/src/types/__tests__/theme-types.test.ts`
  - [ ] Write type assertion tests using TypeScript satisfies operator:
  ...
  - [ ] Run `npm --workspace=packages/shared run test` to execute type tests
  - [ ] Verify all type assertions pass

- [ ] Task 9: Integration verification tests (IV1, IV2, IV3)
  - [ ] Test IV1: Run frontend build (`npm --workspace=apps/web run build`) and verify no
        FormSettings errors
  - [ ] Test IV1: Run backend build (`npm --workspace=apps/api run build`) and verify no
        FormSettings errors
  - [ ] Test IV2: Create temporary test file that imports FormTheme in form-builder.service.ts,
        verify import works
  - [ ] Test IV3: Write integration test that calls GET /api/forms/:id and verifies theme object
        serialization:
